...
public class HeapSort extends Sort {
	private static int out; private static int in; private static long count;
	
	public static void heapSortIma() {
		
		int p; out = getnElemsIma()-1;
		
		for(p = out / 2; p >= 1; --p) { 
			peneiraIma(p, out); ++count; 
			}
		
		for(in = out; in >= 2; --in) { 
			swapIma(1, in); peneiraIma(1, in - 1); ++count; 
		} 
	...
	public static void heapSortInt() { 
		...
		for(in = out; in >= 2; --in) { 						
			swapInt(1, in); peneiraInt(1, in - 1); 						
			++count; 				
		}
