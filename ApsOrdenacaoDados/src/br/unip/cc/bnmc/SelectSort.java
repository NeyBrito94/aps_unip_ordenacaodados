...
public class SelectSort extends Sort { 
	...
	public static void selectSortIma() { 
		for(out = 0; out < getnElemsIma(); 
				out++) { min = out; for(in = out + 1; in < getnElemsIma(); in++) { 
					if(getArrayIma(min).getWidth() > getArrayIma(in).getWidth()) min = in; ++count; 
					} 
				swapIma(out, min); ++count; 
			}
	...
	public static void selectSortInt() { 
		for(out = 0; out < getnElemsInt(); out++) { 
			min = out; for(in = out + 1; in < getnElemsInt(); in++) { 
				if(getArrayInt(min) > getArrayInt(in)) min = in; 
				++count; 
				} 
			swapInt(out, min); 
			++count; 
